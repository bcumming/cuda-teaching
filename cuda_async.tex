...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \begin{enumerate}
        \item Open \lst{util.h} in \lst{cuda/examples/async} and look at the helpers for
        \begin{itemize}
            \item asynchronous copy \lst{copy_to_@\{@host@/@device@\}@_async()}
            \item pinned allocation \lst{malloc_pinned_host()}
        \end{itemize}
        ...
        \item Walk through \lst{memcopy3.cu}
        \begin{itemize}
            \item what effect does changing the number of chunks have?
        \end{itemize}
    \end{enumerate}
